...
2026-09-25 GSE 集合导入顺序修正：锁定 `f225d4c` 的 `Utils.lua:478-510` 明确按 Variables→Sequences→Macros 处理。公开 `/api/gse/inspect` 红灯命令 `.venv/Scripts/python.exe -m pytest tests/sim2gse/test_interface.py -q -k does_not_let_bad_macro_block_preceding_sequence` 修复前 1 failed、83 deselected，原因是最外层坏 `Macros` table 被错误用于阻断已经先存入的独立序列；路径感知修正后同命令 1 passed、83 deselected。坏 `Variables` 对照仍阻断后续序列并使 `/api/tasks` 返回 HTTP 400；只有坏顶层 `Macros` 时独立序列可通过预检并进入 `/api/tasks`，坏对象仍保留兼容性诊断。新增子树回归表明外层 `Macros` 中坏成员只保守阻断该子树内序列；`Sequences` 递归内坏成员因同层 `pairs` 顺序不确定，预检返回明确阻断理由。受影响测试 `.venv/Scripts/python.exe -m pytest tests/sim2gse/test_interface.py -q -k "does_not_let_bad_macro_block_preceding_sequence or scopes_bad_nested_macro_to_its_subtree or blocks_raw_variable_macro_tables_without_upstream_shape"` 为 3 passed、82 deselected。重新逐个 POST 16 个真实文件到公开 `POST /api/gse/inspect`，没有保存原串或逐条响应，统计保持不变：16 decoded、21 成员/版本、15 个不同摘要、5 个兼容性阻断、19 unsupported、2 requires_character_validation，六类控制块计数未变。聚焦套件 `.venv/Scripts/python.exe -m pytest tests/sim2gse/test_interface.py tests/sim2gse/test_program.py -q` 为 120 passed、52 个子检查通过（88.86s）。仍未在实机导入或验收 DPS。
