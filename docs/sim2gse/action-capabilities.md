...
2026-09-13 当前执行规则：不按职业、专精或职责提前拒绝；统一以伤害为目标。身份取原生报告，保留原生战前动作，施法、引导和蓄力交给原生调度。装备组展开只收取原生实际执行过的物品代理，保留原始后台标记；无游戏法术身份的引擎设施不导出为技能按钮。下文的首轮邪恶清单和施法限制属于历史研究，不构成当前白名单。详见[本地入口](local-task.md)。
...
用户随后明确了全自动产品流程：具体角色能力清单由工具代码在运行时生成，本文的技能表只用于开发取证，不是用户需要人工核对的步骤，也不是完成任务 6 前必须手工做出的最终清单。首版战斗药水固定关闭，不提供逐能力配置选项；受支持主动饰品自动处理。有效输入直接经过自动检查、模拟优化和导出；异常输入或未支持能力自动说明原因并停止。详见[全自动生成决议](../../myspec/changes/sim2gse-wayfinder/issues/06-supported-actions.md)及[实施方案](implementation-plan.md)第 5.2 节。
...
以下采用用户 2026-09-13 的基准驱动决议，替代此前“职业能力全集先行”的设计；下方历史来源调查仍作为研究证据，不作为现行运行门禁。

1. 先用当前角色及固定原生策略运行基准，再从整批实际执行的动作中，依据引擎自身的玩家／宠物归属、主动／后台及物品使用标记取得可按动作。基准决定本次候选范围，原生标记决定动作身份；不按伤害统计名称猜测，不维护人工辅助技能排除表。用户输入仍按安全角色字段读取，不开放任意用户动作脚本。
2. 本次基准未使用的其他能力不阻断任务，也不加入搜索空间；这不等于宣布它们在游戏中不可用。不能仅因开场资源不足或冷却未就绪，删除基准后续实际使用过的动作。
3. 候选集合须覆盖本次基准批次；单条样例轨迹仅用于顺序参考。当前列出的历史技能表不是生产白名单，也不是邪恶专精全部主动技能；换版本、天赋、装备或场景后重新取得。
...
历史记录：以下全集检查及人工分类补充已被 2026-09-13 的基准驱动决议替代，不再作为现行产品规则。

固定产品源码 `b845947a34429874433d8e9362326894650dd20a` 中，`engine/dbc/sc_spell_info.cpp` 将属性位 22 显示为跟踪施法目标，属性位 28 才是禁止战斗；`SX_NO_COMBAT`（旧禁止战斗枚举名）仍指向 22。上文历史查询对 22 的解释不能用于当前产品筛选。实现采用同构建的属性位 28，并保留明确排除理由。

用户已确认排除纯移动、控制和辅助技能，有伤害能力仍须支持或报错。候选仍由版本数据和角色自动发现；通用效果分类之外，`projects/sim2gse/compatibility/utility-evidence.json` 保存四项辅助用途的固定原生说明及查询散列。该补充仅决定用途分类，不决定角色是否拥有能力，也不映射输出技能；运行时核对版本和查询散列并报告排除理由，未知能力仍报错。

分类依据是固定版本说明的正向用途，不是“证明服务器没有任何隐式行为”。此前实施过程中对占位及区域效果一律拒绝的判定过严，未作为最终要求。蛮牛冲撞按冲锋与击倒说明归为移动控制，同时保留区域触发11887未完整建模及未实机验证的提示。
