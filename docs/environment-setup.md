...
本机游戏可执行文件和安装版本记录均为正式服 12.1.0.69814。工具和接口资料的具体版本以 `scripts/dev/versions.json` 为准；本机使用仓库内 `.venv`（独立 Python 环境），无需启动编辑器。
...
- Node.js（JavaScript 运行环境）与 Playwright（浏览器自动化工具）：按 `package-lock.json` 准备界面回归依赖。
...
安装脚本只写 `.tools`（工具缓存）、`.venv`（独立环境）与 `node_modules`（Node.js 依赖），不改变系统搜索路径或游戏文件；可重复运行。下载前后核对摘要；Lua 和语言服务器摘要来自官方源站或发行接口，静态检查器摘要为本次上游下载实测值。若版本记录升级导致旧缓存校验失败，保留现场并明确处理对应缓存，不跳过校验。
...
依赖本机研究引擎、私人角色和历史报告的装备规划器、Raidbots（模拟服务）和 Sim2GSE 完整搜索作为本机验收，不进入 PR（拉取请求）云端门禁。静态和模拟测试不能冒充真实游戏验收。

## 接口资料

资料保存在 `.tools/wow-ui-source`（暴雪源码）、`.tools/wow-resources`（接口清单）和 `.tools/wow-api`（类型注解）。源码与资源清单固定到本机匹配的 12.1.0.69814 提交。

**API 注解由本仓库自己生成，不再等上游发版。** 上游 `Ketho/vscode-wow-api` 的更新记录停在 12.0.1（2026-02-24），而它要 Lua 5.4 与 5 个需编译的 C 模块，其 `setup/README.md` 明确劝阻在 Windows 上安装。`scripts/dev/build_wow_annotations.py` 用标准库复刻同一套转换规则，从已固定的暴雪文档产出 `annotations/`，随客户端补丁重跑即可。转换规则只有三条定值映射，产出与上游实现等同。
