-- Fixed upstream compiler at the client boundary; called by the product task.
...
local spellNames = expected.spells
...
GSE.GetClassIDforSpec = function(id) assert(id == expected.identity.spec_id); return expected.identity.class_id end
...
GSE.PerformMergeAction = function(operation, classid, name, value)
    assert(operation == "REPLACE" and classid == expected.identity.class_id and name == expected.name)
