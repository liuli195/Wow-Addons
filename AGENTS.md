...
### wow-addon-test（插件逻辑测试技能）

当任务需要验证插件的生命、主资源、职业资源或专精／形态切换逻辑时，先读取仓库内的
`.agents/skills/wow-addon-test/SKILL.md`，再使用它提供的命令接入和运行真实生产文件。
技能自带的工具、压缩数据、参考资料和自测随技能目录分发；验收只覆盖其声明的逻辑范围，
不把它当作真实客户端、画面或天赋配置验收。最终代码审查和两个代理的任务履约验收由 ChatGPT
通过当前工作区连接完成，结果必须区分已验证、未验证和已知差异。
