...
| `assets/EUI_FacetedMedia/`、`assets/CrosshairHUDMedia/`（素材） | 原始图片、共享媒体素材和预览图片；插件运行所需的内置素材仍随插件保存（准星 HUD 的设计稿与切图在后者） |
| `tests/`（测试） | 全仓插件、项目和工具的测试；执行条件见[测试说明](tests/README.md) |
