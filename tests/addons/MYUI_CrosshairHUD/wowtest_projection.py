...
NOT_COMPARED = {
    ...
    # 实测结论，不是推测：把 UNIT_HEALTH 从订阅里摘掉后，78 个用例的差异仍为 0，
    # 因为 0.1 秒的轮询会兜住一切事件更新，而检查点之间的间隔不小于一次轮询。
    # 也就是说**"事件是否被订阅"这件事本身在这批场景里不可观测**——
    # 本工具能证明的是"读数算得对"，不能证明"事件订阅写对了"。
    "事件订阅本身是否正确": "轮询会兜住事件更新，检查点间隔不小于一次轮询，"
                            "漏订阅事件在本批场景里不产生可观测差异（已实测，非推测）",
}
