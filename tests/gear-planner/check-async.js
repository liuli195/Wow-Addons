...
(async()=>{
 ...
 const queue=[];const merge=vm.createContext({AbortController,originalMatchController:null,model:{gear:{head:'a'}},busy:false,originalItems:{head:{value:'original'}},boot:{slots:[['head']]},originalMatchKey:'',originalMatches:{},api:()=>new Promise(resolve=>queue.push(resolve)),drawGear(){},drawEditor(){}});
 load(merge,'function updateOriginalMatches(', 'function drawGear(');merge.updateOriginalMatches();const oldController=merge.originalMatchController;merge.model={gear:{head:'b'}};merge.updateOriginalMatches();queue[1]({head:[2]});await Promise.resolve();queue[0]({head:[1]});await Promise.resolve();assert.equal(merge.originalMatches.head[0],2);assert(oldController.signal.aborted);merge.model.character={class:'demonhunter',spec:'havoc'};merge.updateOriginalMatches();assert.equal(queue.length,3);
