...
(async()=>{
 ...
 const samples=JSON.parse(fs.readFileSync(path.join(PROJECT,fixtureDir,'manifest.json'),'utf8')).samples;
 try{for(const sample of samples){
  ...
  try{
   ...
   const input=fs.readFileSync(path.join(PROJECT,fixtureDir,sample.spec+'.input.simc'),'utf8').split(/\r?\n/).filter(line=>keys.has(line.split('=')[0])).join('\n');
