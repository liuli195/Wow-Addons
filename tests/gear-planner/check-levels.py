...
sys.path.insert(0, str(Path(__file__).resolve().parents[2] / "projects/gear-planner"))
import copy,json
from pathlib import Path
import server
REPORTS = server.ROOT / ".local/tests/gear-planner"
...
(REPORTS / 'level-check-results.json').write_text(json.dumps({'catalogProducts':len(covered),'nativeCases':native},indent=2),encoding='utf-8')
