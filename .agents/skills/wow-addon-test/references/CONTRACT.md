...
工具的输入在 `assets/data/cases.json.gz`；预期在 `assets/data/baselines.json.gz`，普通运行只读且校验摘要。适配器不会收到预期或用于参照路由的 `resource_kind`。生成器知道哪段原生代码对应哪个用例；被测代码必须自己通过实际接口、职业、专精和形态选择资源。
...
执行顺序：安装初始输入 → 真正启动插件 → 每步先让上一检查点的输入推进到该步时间 → 安装该步完整输入 → 投递该步事件 → 推进约定等待 → 再排空一轮零延迟回调 → 读取业务输出。
...
参考侧与实际侧**必须使用同一个「观测时刻」定义**，否则比较的是两个不同瞬间的状态。
执行顺序是：先让上一检查点的输入推进到该步时间 → 安装该步完整输入 → 投递该步事件 → 推进约定等待 → 排空一轮零延迟回调 → 读取实际输出；
参考侧在**该步时间**重新采样。因此：

| 情况 | 正确处理 |
| --- | --- |
| **模拟器漏执行计时器、时间顺序错误** | 修运行器；并用独立时序测试证明（见下） |
| **事件、登录延迟或去抖确实需要等待** | 在用例的 `settle` 里**声明允许的等待时长**；执行生产注册的回调；参考与实际采用同一观测时刻定义 |
| **连续变化的资源（如充能）靠周期刷新** | 明确采样时刻，或声明**允许的数据陈旧上限**；不得把任意检查点都当成"刚刚刷新过" |

**误差界限只能按量推导，不许笼统写一个百分比。** 对线性充能，刷新延迟对应的比例误差为：

    比例误差 ≤ 允许的陈旧时间 ÷ 参考充能时长

它只适用于相应的连续量，**不能**用来放过资源类型、就绪状态、缺少更新或超期不刷新的错误。
界限**以内**的合法行为必须仍然通过，界限**以外**的延迟与错误必须能被检出。

**两条固定回归**（防止时序缺陷被"多等一会儿"掩盖）：

1. 跨越到下一步输入时，**较早到期的计时器不得提前读到未来状态**；
2. `settle > 0` 时，参考侧、实际侧与下一步输入的时间必须一致——不得出现参考停在等待之前、
   实际已经推进，或下一步导致虚拟时间倒退。

**仍然禁止**：为了保证通过而在每个检查点强制刷新生产代码（那会掩盖漏注册事件）。
